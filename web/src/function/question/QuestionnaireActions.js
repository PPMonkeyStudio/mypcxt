...
export const set_addServiceModelUnitList = (unitList) => ({type: 'set_addServiceModelUnitList', unitList: unitList,})
...
export const addServiceDefinition = (addServiceModelState) => {
  return(dispatch) => {
    let formData = new FormData();
    formData.append("ServiceDefinition.service_definition_describe", addServiceModelState.service_definition_describe);
    formData.append("ServiceDefinition.service_definition_unit", addServiceModelState.service_definition_unit);
    fetch('/mypcxt/Service/addServiceDefinition', {
      method: 'POST',
      body: formData
    }).then((response) => {
      if (response.status === 200) {
        response.json().then((responseJson) => {
          dispatch(set_addServiceModalVisible(false));
          dispatch(getQuestionnaireVO());
        }).catch((error) => {
          console.error(error);
        });
      } else {
...
export const listUnitAll_toAddServiceModel = () => {
  return(dispatch) => {
    fetch('/mypcxt/Unit/listUnitAll', {method: 'POST'}).then((response) => {
      if (response.status === 200) {
        response.json().then((responseJson) => {
          dispatch(set_addServiceModelUnitList(responseJson));
        }).catch((error) => {
          console.error(error);
        });
      } else {
