...
export const getquestionnaireDTO_byServiceDefinitionID = (mypcxt_service_definition_id) => {
  return(dispatch) => {
    ...
    formData.append("questionnaireDTO.serviceDefinitionDTO.serviceDefinition.mypcxt_service_definition_id", mypcxt_service_definition_id);
    fetch('/mypcxt/Question/getquestionnaireDTO_byServiceDefinitionID', {
      method: 'POST',
      body: formData,
    }).then((response) => {
      if (response.status === 200) {
        response.json().then((responseJson) => {}).catch((error) => {
          console.error(error);
        });
      } else {
        console.error(response.status);
